...
@if($child->video)
    <x-media.video :video="$child->video"/>
@elseif($child->hasGallery())
    <div class="gallery-owl owl-carousel owl-theme-inside">
        @foreach($child->gallery()->orderBy('sort')->get() as $slider)
...
@else
    <img src="{!! ImgHelper::get_cached($child->image, config('maguttiCms.image.medium')) !!}"
         alt="{{ $child->title }}" border="0" class="img-fluid w-100">
@endif
