...
d.inputTemplate=b('<input type="file">').css({"font-size":f.height+"px",opacity:0,position:"absolute",right:"-3px",top:"-3px","z-index":999});d.createInput=function(){var j=d.inputTemplate.clone();var k=j.name="input"+d.inputCount++;if(f.multi){j.attr("multiple",!0)}
...
m.preventDefault();m.stopPropagation()};d.fileExistsInQueue=function(k){for(var j in d.inputs){input=d.inputs[j];limit=input.files.length;for(var l=0;l<limit;l++){existingFile=input.files[l];if(existingFile.name==k.name&&!existingFile.complete){return!0}}}
...
d.addQueueItem=function(k){if(b.inArray("onAddQueueItem",f.overrideEvents)<0){d.removeExistingFile(k);k.queueItem=d.queueItem.clone();k.queueItem.attr("id",f.id+"-file-"+d.fileID++);k.queueItem.find(".close").bind("click",function(){a.cancel.call(g,k);return!1});var m=k.name;if(m.length>f.truncateLength&&f.truncateLength!=0){m=m.substring(0,f.truncateLength)+"..."}
...
if(f.fileType){if(b.isArray(f.fileType)){var j=!1;for(var l=0;l<f.fileType.length;l++){if(k.type.indexOf(f.fileType[l])>-1){j=!0}}
...
return k};d.checkExists=function(k){if(b.inArray("onCheck",f.overrideEvents)<0){b.ajaxSetup({async:!1});var j=b.extend(f.formData,{filename:k.name});b.post(f.checkScript,j,function(l){k.exists=parseInt(l)});if(k.exists){if(!confirm("A file named "+k.name+" already exists in the upload folder.\nWould you like to replace it?")){a.cancel.call(g,k);return!0}}}
if(typeof f.onCheck==="function"){f.onCheck.call(g,k,k.exists)}
return!1};d.uploadFile=function(k,l){if(!k.skip&&!k.complete&&!k.uploading){k.uploading=!0;d.uploads.current++;d.uploads.attempted++;xhr=k.xhr=new XMLHttpRequest();if(typeof FormData==="function"||typeof FormData==="object"){var m=new FormData();m.append(f.fileObjName,k);for(i in f.formData){m.append(i,f.formData[i])}
xhr.open(f.method,f.uploadScript,!0);xhr.upload.addEventListener("progress",function(n){if(n.lengthComputable){d.progress(n,k)}},!1);xhr.addEventListener("load",function(n){if(this.readyState==4){k.uploading=!1;if(this.status==200){if(k.xhr.responseText!=="Invalid file type."){d.uploadComplete(n,k,l)}else{d.error(k.xhr.responseText,k,l)}}else{if(this.status==404){d.error("404_FILE_NOT_FOUND",k,l)}else{if(this.status==403){d.error("403_FORBIDDEN",k,l)}else{d.error("Unknown Error",k,l)}}}}});xhr.send(m)}else{var j=new FileReader();j.onload=function(q){var t="-------------------------"+(new Date).getTime(),p="--",o="\r\n",s="";s+=p+t+o;s+='Content-Disposition: form-data; name="'+f.fileObjName+'"';if(k.name){s+='; filename="'+k.name+'"'}
...
s+=p+t+p+o;xhr.upload.addEventListener("progress",function(u){d.progress(u,k)},!1);xhr.addEventListener("load",function(v){k.uploading=!1;var u=this.status;if(u==404){d.error("404_FILE_NOT_FOUND",k,l)}else{if(k.xhr.responseText!="Invalid file type."){d.uploadComplete(v,k,l)}else{d.error(k.xhr.responseText,k,l)}}},!1);var n=f.uploadScript;if(f.method=="get"){var r=b(f.formData).param();n+=r}
xhr.open(f.method,f.uploadScript,!0);xhr.setRequestHeader("Content-Type","multipart/form-data; boundary="+t);if(typeof f.onUploadFile==="function"){f.onUploadFile.call(g,k)}
...
j.skip=!0;if(l=="404_FILE_NOT_FOUND"){d.uploads.errors++}else{d.queue.errors++}
if(k){a.upload.call(g,null,!0)}};d.uploadComplete=function(l,j,k){if(b.inArray("onUploadComplete",f.overrideEvents)<0){j.queueItem.find(".progress-bar").css("width","100%");j.queueItem.find(".fileinfo").html(" - Completed");j.queueItem.find(".progress").slideUp(250);j.queueItem.addClass("complete")}
...
if(f.dnd){var h=f.dropTarget?b(f.dropTarget):d.queueEl.get(0);h.addEventListener("dragleave",function(j){j.preventDefault();j.stopPropagation()},!1);h.addEventListener("dragenter",function(j){j.preventDefault();j.stopPropagation()},!1);h.addEventListener("dragover",function(j){j.preventDefault();j.stopPropagation()},!1);h.addEventListener("drop",d.drop,!1)}
...
return!1}})},debug:function(){return this.each(function(){console.log(b(this).data("uploadifive"))})},clearQueue:function(){this.each(function(){var f=b(this),c=f.data("uploadifive"),e=c.settings;for(var d in c.inputs){input=c.inputs[d];limit=input.files.length;for(i=0;i<limit;i++){file=input.files[i];a.cancel.call(f,file)}}
...
d.skip=!0;e.filesCancelled++;if(d.uploading){e.uploads.current--;d.uploading=!1;d.xhr.abort();delete d.xhr;a.upload.call(g)}
...
b("#"+f.queueID).find(".uploadifive-queue-item").not(".error, .complete").each(function(){_file=b(this).data("file");if((e.uploads.current>=f.simUploadLimit&&f.simUploadLimit!==0)||(e.uploads.current>=f.uploadLimit&&f.uploadLimit!==0)||(e.uploads.count>=f.uploadLimit&&f.uploadLimit!==0)){return!1}
if(f.checkScript){_file.checking=!0;skipFile=e.checkExists(_file);_file.checking=!1;if(!skipFile){e.uploadFile(_file,!0)}}else{e.uploadFile(_file,!0)}});if(b("#"+f.queueID).find(".uploadifive-queue-item").not(".error, .complete").length==0){e.queueComplete()}}else{if(e.uploads.current==0){if(b.inArray("onError",f.overrideEvents)<0){if(e.filesToUpload()>0&&f.uploadLimit!=0){alert("The maximum upload limit has been reached.")}}
